...
from traffic_vision.entity.config_entity import DataValidationConfig
...
class DataValidation:
    ...
    def validate_yaml_file(self) -> bool:
        ...
        try:
            ...
            # Check if classes match expected vehicle classes
            if 'names' in data:
                logging.info(f"Classes in data.yaml: {data['names']}")
                # Update class names for vehicle detection if needed
                if 'Glass' in data['names'] or 'Waste' in data['names']:
                    logging.warning("Updating class names for vehicle detection")
                    data['names'] = ['Car', 'Motorcycle', 'Bus', 'Truck', 'Other']
